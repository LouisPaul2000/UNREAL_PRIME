...
class SpaceGameWindow(window.Window):
    def __init__(self, *args, **kwargs):
        ...
        self.monster_image = image_loads("monsts.png")                 # monster image data
        self.background = Sprite('space2.jpeg')                          # background image data
        self.monster_bullet_image = image_loads('BALLS.png')
    ...
    def main_loop(self):
        ...
        title_text.text = 'SHOOTER'
        self.start_text.text = 'Start'
        #self.music = pyglet.resource.media('Ritviz.wav')
        #music.play()
        #pyglet.media.load('BACKscore.mp3').play()
        ...
        if self.states.shoot_flag and Bullet.bullet_count_control % 7 == 0:  # control the bullet counts
            ...
            if not self.states.trible_fire:
                self.bullets.append(Bullet(self.ship
                                           , self.bullet_image
                                           , self.height
                                           , x=self.ship.x + (self.ship.image.width / 4) -
    ...
    def create_monster(self, interval):
        if len(self.monsters) < self.max_monsters:
            ...
            self.monster_bullets.append(Bullet('', self.monster_bullet_image, self.height,x=x, y=y))
